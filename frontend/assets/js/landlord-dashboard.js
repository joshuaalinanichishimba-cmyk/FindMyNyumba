...
window.fetchStats = async function() {
    try {
        const res = await fetch('https://findmynyumba-backend.onrender.com/api/v1/landlord/stats');
        if (res.ok) {
...
window.handleVerifyUpload = async function(input, type) {
    ...
    try {
        const res = await fetch('https://findmynyumba-backend.onrender.com/api/v1/landlord/upload-doc', { method: 'POST', body: formData });
        if (res.ok) {
...
window.loadLandlordMessages = async function() {
    ...
    try {
        const res = await fetch(`https://findmynyumba-backend.onrender.com/api/v1/messages/${window.activeChatId}`);
        if(res.ok) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(chatForm) {
        chatForm.addEventListener('submit', async (e) => {
            ...
            await fetch('https://findmynyumba-backend.onrender.com/api/v1/messages', {
                method: 'POST',
...
window.sendLandlordMessage = async function(e) {
    ...
    // 2. Send to Backend
    try {
        const res = await fetch('https://findmynyumba-backend.onrender.com/api/v1/messages', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
window.loadNotifications = async function() {
    try {
        const res = await fetch('https://findmynyumba-backend.onrender.com/api/v1/notifications/landlord/1');
