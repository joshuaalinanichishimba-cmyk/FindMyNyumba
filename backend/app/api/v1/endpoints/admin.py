...
def get_analytics(admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    """Admin BI dashboard data. Real aggregates only. SearchLog queries guarded."""
    ...
    users_total = db.query(F.count(User.id)).scalar() or 0
    by_role = dict(db.query(User.role, F.count(User.id)).group_by(User.role).all())

    listings_total = db.query(F.count(Listing.id)).scalar() or 0
    by_status = dict(db.query(Listing.status, F.count(Listing.id)).group_by(Listing.status).all())

    by_kind = dict(db.query(ListingEvent.kind, F.count(ListingEvent.id)).group_by(ListingEvent.kind).all())
    views = by_kind.get("view", 0); contacts = by_kind.get("contact", 0); saves = by_kind.get("save", 0)

    viewings_total = db.query(F.count(ViewingRequest.id)).scalar() or 0
    viewings_completed = db.query(F.count(ViewingRequest.id)).filter(
        ViewingRequest.status == ViewingStatus.COMPLETED.value).scalar() or 0

    reviews_total = db.query(F.count(Review.id)).scalar() or 0
    reviews_approved = db.query(F.count(Review.id)).filter(Review.status == "approved").scalar() or 0

    top_viewed = (db.query(Listing.id, Listing.title, F.count(ListingEvent.id).label("v"))
                    .join(ListingEvent, ListingEvent.listing_id == Listing.id)
                    .filter(ListingEvent.kind == "view")
                    .group_by(Listing.id, Listing.title)
                    .order_by(F.count(ListingEvent.id).desc()).limit(8).all())

    top_terms, top_unis = [], []
    try:
        from app.models.search_log import SearchLog
        top_terms = (db.query(SearchLog.query, F.count(SearchLog.id))
                       .filter(SearchLog.query.isnot(None))
                       .group_by(SearchLog.query).order_by(F.count(SearchLog.id).desc()).limit(10).all())
        top_unis = (db.query(SearchLog.university, F.count(SearchLog.id))
                      .filter(SearchLog.university.isnot(None))
                      .group_by(SearchLog.university).order_by(F.count(SearchLog.id).desc()).limit(10).all())
    except Exception:
        pass

    return {
        "users": {"total": users_total, "by_role": by_role},
        "listings": {"total": listings_total, "by_status": by_status},
        "engagement": {"views": views, "contacts": contacts, "saves": saves},
        "viewings": {"total": viewings_total, "completed": viewings_completed},
        "reviews": {"total": reviews_total, "approved": reviews_approved},
        "funnel": {"views": views, "contacts": contacts, "completed_viewings": viewings_completed},
        "top_viewed": [{"id": i, "title": t, "views": v} for (i, t, v) in top_viewed],
        "top_searches": [{"term": q, "count": c} for (q, c) in top_terms],
        "top_universities": [{"name": u, "count": c} for (u, c) in top_unis],
    }
