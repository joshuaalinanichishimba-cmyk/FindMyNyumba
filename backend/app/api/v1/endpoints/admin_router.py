...
def reject_verification(user_id: int, payload: RejectPayload, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")
...
def get_growth(admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    for i in range(5, -1, -1):
        ...
        while m <= 0: m += 12; y -= 1
        months.append((y, m))
    def label(y, m): return datetime(y, m, 1).strftime("%b %Y")
    ...
    for (dt,) in all_users:
        if dt: user_counts[(dt.year, dt.month)] += 1
    for (dt,) in all_listings:
        if dt: listing_counts[(dt.year, dt.month)] += 1
    return {
...
def send_announcement(payload: AnnouncementPayload, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    target_roles = ["student", "landlord", "student_host"] if payload.target == "all" else [payload.target]
    users = db.query(User).filter(User.role.in_(target_roles), User.is_active == True).all()
    for user in users:
        msg = Message(sender_id = admin.id, receiver_id = user.id, content = f"[ANNOUNCEMENT] {payload.title}\n\n{payload.body}", is_read = False)
        db.add(msg)
    db.commit()
...
def update_settings(payload: SettingsPayload, admin: User = Depends(require_admin)):
...
def change_password(payload: AdminChangePasswordPayload, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
